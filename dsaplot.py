...
strmS = ax0.streamplot(x1,y1, U, V, start_points=start, color="crimson", linewidth=2) 
...
ax0.fill_between(lx,line(lx),zero(lx),where=line(lx)>zero(lx),facecolor = 'grey', 
                 alpha = 0.2,edgecolor = 'blue' ,interpolate=True)
...
def Cosmological_Parameters(m,r):
   ...
   # plt.hlines(-0.67,0,t_max,color = 'blue',linestyle = '-.',linewidth = 0.5)
    ...
    plt.vlines(t[current_time_index],-1,1,color = 'black',linestyle = '--',
               linewidth = 1)
    plt.tight_layout()
